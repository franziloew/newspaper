...
In this specification, the function $f(W_t)$ is specified as $\beta_2W_t+\beta_3W_t*T_t$. The terms $W_t$ and $W_t*T_t$ should absorb
any smooth relationship between the date and $\epsilon_t$ in the days surrounding the election day. If the RD assumption is valid (i.e., $\epsilon_t$ does not change discontinuously at the election day) the estimate of $\beta_1$, the coefficient of interest, will be unbiased even without further controls. However, in the following section dummy variables for each party $k$ ($D_{t,k-1}$) are included in the model to estimate the isolated effect for each news paper / party pair.

$$
\ln(\text{CS}_{t})=\beta_0+\beta_1T_t+\beta_2W_{t}+\beta_3D_{t,k-1}+\beta_4T_tD_{t,k-1}+\epsilon_t
$$

Including interaction terms $T_tD_{t,k-1}$ - and since we are estimating an isolated model for each newspaper - $\beta_4$ gives the average treatment effect for each newspaper/party pair (i.e. how how much the slope changes for each party).
<!-- Todo: Define why we need the interaction term between treatment + dummy (find literature!) -->
