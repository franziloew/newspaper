...
group_colors <- c(AfD = "#009ee0", Bild.de = "#CC6600", Handelsblatt ="#9633FF")

p2 <- plot_topic_distr_time(9, source_filter, group_colors) + theme(legend.position = 'none')
p3 <- plot_topic_distr_time(38, source_filter, group_colors) + labs(title = NULL)
...
p1 <- theta %>% 
  ...
  top_n(n = 10, wt = theta_mean) %>% 
  ggplot(aes(reorder(topic, theta_mean), theta_mean, fill = source))+
  geom_col(show.legend = F, fill="#ffed00", alpha = 0.7 ) +
  coord_flip() +
  labs(x = NULL, y = 'Topic proportion', 
       title = 'Mean topic proportion of FDP press releases',
       subtitle = 'Top 15 topics with highest probability') +
  theme(axis.title = element_text(size = 4))

group_colors <- c(FDP = "#ffed00", Bild.de = "#CC6600", Handelsblatt ="#9633FF")
source_filter <- c('FDP', 'Bild.de', 'Handelsblatt')
p2 <- plot_topic_distr_time(39, source_filter, group_colors) + theme(legend.position = 'none')
p3 <- plot_topic_distr_time(10, source_filter, group_colors) + labs(title = NULL)
