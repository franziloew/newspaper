...
theta <- as.data.frame(stmModel$theta) %>% # get all theta values for each document
  ...
  left_join(., model_df %>% 
              select(date,type,source,doc_index,title), by="doc_index") %>% 
  select(doc_index, topic, theta, topic_label, date, type, source, title)

plot_sample_document <- function(doc_id){
...
sample1 <- 6397
p1 <- plot_sample_document(sample1)

# Wahlen (ZEIT ONLINE)
sample2 <- 5833
p2 <- plot_sample_document(sample2)
...
# Linke Gefahr (AfD)
sample3 <- 17511
p1 <- plot_sample_document(sample3)

# Trump (B90/Gruene)
sample4 <- 17600
p2 <- plot_sample_document(sample4)
